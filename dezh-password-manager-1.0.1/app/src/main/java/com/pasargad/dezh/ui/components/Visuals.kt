...
/**
 * Category avatar styling: a stable tonal pair (container/onContainer) for a
 * letter monogram — the clean contacts-list pattern. Colors stay inside the
 * warm brand family (primary/secondary/neutral containers); the lapis accent
 * is deliberately NOT used here so avatars never fight the brand gold.
 */
...
private fun styleFor(family: AvatarFamily, category: String): CategoryAvatarStyle = when (family) {
    AvatarFamily.FINANCE, AvatarFamily.SOCIAL -> CategoryAvatarStyle(
    ...
    AvatarFamily.HASHED -> when (kotlin.math.abs(category.hashCode()) % HASH_ROTATION) {
        ...
        else -> CategoryAvatarStyle(MaterialTheme.colorScheme.surfaceContainerHighest, MaterialTheme.colorScheme.onSurfaceVariant, null)
    }
}
