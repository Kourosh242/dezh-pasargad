...
class RoomVaultEntryRepository(
    ...
    override fun observeEntry(id: String): Flow<VaultEntry?> =
        dao.observeById(id).map { row -> row?.let { decryptRowContained(it) } }.flowOn(ioDispatcher)

    override suspend fun getEntry(id: String): VaultEntry? = withContext(ioDispatcher) {
        dao.getById(id)?.let { decryptRowContained(it) }
    }
    ...
    /**
     * Containment wrapper: corrupted/unrecoverable rows degrade to null instead
     * of killing the whole flow, while a LOCKED session still fails closed
     * (VaultSessionLockedException always propagates — security invariant).
     */
    private fun decryptRowContained(row: VaultEntryEntity): VaultEntry? = try {
        decryptRow(row)
    } catch (locked: com.pasargad.dezh.security.VaultSessionLockedException) {
        throw locked
    } catch (@Suppress("TooGenericExceptionCaught", "SwallowedException") failure: Exception) {
        // Deliberate containment: the corrupted row is skipped, not surfaced.
        null
    }
    ...
    private fun filterDecrypted(rows: List<VaultEntryEntity>, query: String): List<VaultEntry> {
        ...
        // Containment: an unreadable (corrupted/unrecoverable) row is skipped so
        // one bad record can never blank the vault or crash the list flow.
        val entries = rows.mapNotNull { row -> decryptRowContained(row) }
