...
fun EntryDetailScreen(
    ...
    // ONE copy window for EVERY copied field (password/username/email):
    // each copy re-arms the wipe, the newest always supersedes older pending
    // wipes, and when it fires the clipboard is cleared and every badge hides.
    // (Split per-field timers let an older wipe clear a newer copy early.)
    var copiedTarget by remember { mutableStateOf<String?>(null) }
        ...
        Column(
            ...
            HeroCard {
                Row(
                    ...
                    EntryMonogram(entry.title, size = 64.dp)
                    Column(
                        modifier = Modifier
                            .weight(1f)
                            .padding(start = 12.dp),
            ...
            val copiedFieldLabel = stringResource(R.string.generator_copied)
            val copyUsernameDescription = stringResource(R.string.entry_copy_username)
            val copyEmailDescription = stringResource(R.string.entry_copy_email)
            if (entry.username.isNotBlank()) {
                AccountBlock(
                    label = stringResource(R.string.entry_field_username),
                    value = entry.username,
                    copyDescription = copyUsernameDescription,
                    copied = copiedTarget == "username",
                    copiedLabel = copiedFieldLabel,
                    onCopy = {
                        clipboard.setText(AnnotatedString(entry.username))
                        copiedTarget = "username"
                        copyGeneration += 1
                    },
                )
            }
            if (entry.email.isNotBlank()) {
                AccountBlock(
                    label = stringResource(R.string.entry_field_email),
                    value = entry.email,
                    copyDescription = copyEmailDescription,
                    copied = copiedTarget == "email",
                    copiedLabel = copiedFieldLabel,
                    onCopy = {
                        clipboard.setText(AnnotatedString(entry.email))
                        copiedTarget = "email"
                        copyGeneration += 1
                    },
                )
...
/** Hero card (M3Y): the largest radius, hosts the identity header. */
@Composable
...
/**
 * Standalone tinted info block (M3Y style): label + value + a real copy
 * action, per the "CTA must act" product rule.
 */
@Composable
