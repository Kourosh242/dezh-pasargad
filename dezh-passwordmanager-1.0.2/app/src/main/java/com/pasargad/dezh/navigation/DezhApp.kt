...
import com.pasargad.dezh.presentation.update.UpdateScreen
import com.pasargad.dezh.presentation.update.UpdateViewModel
...
/**
 * The single nav transform: a calm vertical drift + fade when motion is
 * enabled; an instant swap when the user disabled پویانمایی.
 */
private fun navTransform(motionEnabled: Boolean, pop: Boolean): ContentTransform =
    if (!motionEnabled) {
        ContentTransform(fadeIn(snap()), fadeOut(snap()))
    } else if (!pop) {
        ContentTransform(
            fadeIn(tween(NAV_FADE_IN_MS, easing = FastOutSlowInEasing)) +
                slideInVertically(tween(NAV_SLIDE_MS, easing = FastOutSlowInEasing)) { it / NAV_SLIDE_DIVISOR },
            fadeOut(tween(NAV_FADE_OUT_MS, easing = FastOutSlowInEasing)),
        )
    } else {
        ContentTransform(
            fadeIn(tween(NAV_POP_FADE_IN_MS, easing = FastOutSlowInEasing)),
            fadeOut(tween(NAV_POP_FADE_OUT_MS, easing = FastOutSlowInEasing)) +
                slideOutVertically(tween(NAV_SLIDE_MS, easing = FastOutSlowInEasing)) { it / NAV_SLIDE_DIVISOR },
        )
    }

/** Nav transition timings (ms) — calm vertical drift; springs stay touch-only. */
private const val NAV_FADE_IN_MS = 260
private const val NAV_FADE_OUT_MS = 140
private const val NAV_POP_FADE_IN_MS = 220
private const val NAV_POP_FADE_OUT_MS = 200
private const val NAV_SLIDE_MS = 260
private const val NAV_SLIDE_DIVISOR = 14
...
fun DezhApp(container: AppContainer, windowWidthSizeClass: WindowWidthSizeClass, modifier: Modifier = Modifier) {
    ...
    val openVaultTab: () -> Unit = { backStack.clear(); backStack.add(DezhDestination.Vault()); Unit }
    val openGeneratorTab: () -> Unit = { backStack.clear(); backStack.add(DezhDestination.Generator); Unit }
    val openSettingsTab: () -> Unit = { backStack.clear(); backStack.add(DezhDestination.Settings); Unit }
    ...
    // The user's پویانمایی preference gates ALL decorative motion (nav
    // transitions included) — same Local the screens consume.
    ...
    NavDisplay(
        ...
        modifier = modifier,
        // Vertical drift + fade (RTL-neutral): calm emphasized transition,
        // ~300ms on the standard easing — springs stay reserved for touch.
        transitionSpec = { navTransform(motionEnabled, pop = false) },
        popTransitionSpec = { navTransform(motionEnabled, pop = true) },
        predictivePopTransitionSpec = { _ -> navTransform(motionEnabled, pop = true) },
        entryProvider = entryProvider {
            ...
            entry<DezhDestination.Vault> { key ->
                VaultListScreen(
                    ...
                    onCheckForUpdatesClick = { backStack.add(DezhDestination.UpdateCheck) },
                )
            }
            ...
            entry<DezhDestination.Generator> {
                GeneratorScreen(
                    ...
                    onVaultTab = openVaultTab,
                    onSettingsTab = openSettingsTab,
            ...
            entry<DezhDestination.Settings> {
                SettingsScreen(
                    onVaultTab = openVaultTab,
                    onGeneratorTab = openGeneratorTab,
            ...
            entry<DezhDestination.UpdateCheck> {
                UpdateScreen(
                    viewModel = viewModel {
                        UpdateViewModel(
                            checker = container.updateChecker,
                            downloader = container.apkDownloader,
                            verifier = container.apkVerifier,
