...
/**
 * Material 3 top app bar of the vault: search + overflow carry secondary
 * actions; settings lives in the bottom navigation bar (3-tab layout).
 */
...
class VaultTopBarUiTest {
    ...
    fun search_icon_and_overflow_menu_navigate() {
        ...
        // Settings moved out of the overflow menu into the bottom navigation bar.
        composeRule.onNodeWithText("تنظیمات").assertIsDisplayed().performClick()
        assertTrue(box.settingsOpened)

        // Generator moved to the bottom navigation bar (center key dock).
