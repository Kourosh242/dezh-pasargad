...
class EntryScreensRenderTest {
    ...
    fun add_entry_generate_password_fills_the_field() {
        ...
        // 1.0.2: generate moved to a key-glyph IconButton + quick generator
        // sheet; the form field only fills after adopting from the sheet.
        composeRule.onNodeWithContentDescription("تولید").performClick()
        composeRule.waitUntil(timeoutMillis = 5_000) {
            viewModel.quickGenerator.value.password.isNotEmpty()
        }
        composeRule.onNodeWithText("استفاده از این رمز").performClick()
        composeRule.waitUntil(timeoutMillis = 5_000) {
