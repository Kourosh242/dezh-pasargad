...
class InstallOutcomeMappingTest {
    ...
    @Test
    fun `status filter matches our own no-data broadcast`() {
        // REGRESSION (1.0.1 round 2): the filter demanded a "package" data
        // scheme while our status broadcast carries NO data Uri — the system's
        // STATUS_PENDING_USER_ACTION broadcast never matched, the confirmation
        // dialog never launched, and the install silently stalled.
        val filter = PackageApkInstaller.installStatusFilter()
        org.junit.Assert.assertTrue(filter.hasAction(PackageApkInstaller.ACTION_INSTALL_COMPLETE))
        org.junit.Assert.assertEquals(0, filter.countDataSchemes())
        org.junit.Assert.assertEquals(0, filter.countDataAuthorities())
    }
